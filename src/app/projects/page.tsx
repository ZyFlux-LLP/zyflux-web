...
import { ArrowRight, ExternalLink, Users, Star, Filter, X, Award, TrendingUp, Building,Target, Clock, CheckCircle, Phone} from 'lucide-react';
      ...
      <div className="p-8 pb-6">
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 ${project.image} rounded-xl flex items-center justify-center`}>
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${project.status === 'Live' ? 'bg-white text-black' : 'bg-gray-600 text-white'
            }`}>
            {project.status}
          </span>
        </div>
    ...
    Start Your Project
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
            {[
              ...
              { number: "300%", label: "Avg. Growth", icon: <TrendingUp className="w-6 h-6" /> },
              { number: "98%", label: "Success Rate", icon:   <Star className="w-5 h-5 text-yellow-400 fill-current" /> }
            ].map((stat, index) => (
              <div key={index} className="text-center group">
                <div className="text-4xl font-bold text-white mb-2 group-hover:text-gray-400 transition-colors">
                  {stat.number}
                </div>
                ...
                <div className="absolute top-full left-4 right-4 bg-gray-900 border border-gray-700 rounded-xl mt-2 p-4 z-50">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-lg mb-2 transition-all duration-300 ${selectedCategory === category.id
                        ? 'bg-white text-black'
